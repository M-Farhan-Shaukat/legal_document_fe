...
(function () {
  ...
  const isHTMLElement = (element) =>
    isElement(element) && isPrototypeOf(element.dom);
  ...
  const isContentEditableFalse = (node) =>
    isHTMLElement(node) && getRaw(node) === "false";
  ...
  const filterEditableDescendants = (scope, predicate, editable) => {
    ...
    const isEditable = (node) =>
      isWrappedNbsp(node.dom) || !isContentEditableFalse(node);
    each$1(children, (x) => {
      if (editable && isEditable(x) && predicate(x)) {
        result = result.concat([x]);
      }
      result = result.concat(
        filterEditableDescendants(x, predicate, isChildEditable(x, editable))
      );
    });
    return result;
  };
